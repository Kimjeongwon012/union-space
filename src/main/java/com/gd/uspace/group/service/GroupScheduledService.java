...
public class GroupScheduledService {
	...
	public void checkWarningCount() {
		// 경고 횟수가 5의 배수이고
		List<PenaltyDTO> user_list = groupdao.getUsersWarningCount();
		for (PenaltyDTO p : user_list) {
			// 로그인 제한 해제
			groupdao.setUserStatus(p.getUser_id(), 1);			
		}
	}
	...
	public void checkGroupConfirm() {
		...
		for (GroupDTO g : list) {
			...
			if (g.getGroup_people() >= g.getGroup_lowpeople()) {
				...
				List<GroupMemberDTO> groupMember = groupdao.getGroupMembers(g.getGroup_no());
